...
function getApiBase(): string {
  if (typeof window === "undefined") return "/civbro/api";
  const path = window.location.pathname;
  const match = path.match(/^(.*\/civbro)(?:\/index\.html|\/)?$/);
  if (match) {
    return match[1] + "/api";
  }
  return "/civbro/api";
...
async function fetchApi<T>(
  ...
  const url = `${apiBase}${endpoint}${sep}_t=${Date.now()}`;
...
export async function getSettings(): Promise<Record<string, unknown> | null> {
  try {
    // Must be awaited inside the try: returning the promise would let the
    // rejection escape past this catch and surface as an unhandled failure.
    return await fetchApi("/settings/_all");
  } catch {
    return null;
  }
}
