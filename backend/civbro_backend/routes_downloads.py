...
def register_download_routes(app: Any) -> None:
    ...
    async def start_download(request: Request):
        ...
        try:
            ...
            target_dir = config.get_model_dir(subdir)
            if requested_dir:
                resolve_download_path(requested_dir, file_name)
                # The backend's file/model type map is authoritative; clients
                # can send stale category defaults, but never escape the jail.
            resolve_download_path(target_dir, file_name)
    ...
    async def cancel_download(download_id: str):
        ...
        # remove_entry takes _schedule_lock itself; asyncio.Lock is not
        # reentrant, so this must not be wrapped in another acquisition.
