...
from .rust_facade import optimize_image_url
...
def resolve_download_path(download_dir: str, file_name: str) -> str:
    """Jail a download target inside allowed model roots.

    Raises ValueError on any attempt to escape allowed model roots or to smuggle
    path separators through the file name.
    """
    if not isinstance(file_name, str) or file_name in ("", ".", "..") or "\0" in file_name or "\\" in file_name or "/" in file_name:
        raise ValueError(f"Illegal file name: {file_name!r}")

    allowed_roots = config.get_allowed_model_roots()
    target_dir = Path(download_dir).resolve() if download_dir else allowed_roots[0]
    ...
    if not is_allowed:
        raise ValueError(f"Download dir escapes allowed model roots: {download_dir!r}")
...
async def _process_download(download_id: str) -> None:
    ...
    try:
        # Everything that can fail must stay inside this block: _process_download
        # runs as a bare task, so an escaping exception would leave the entry
        # stuck in "downloading" and never free its concurrency slot.
