...
from .config import CIVITAI_REST_API, TYPE_BY_DIR
...
def register_local_routes(app: Any) -> None:
    ...
    async def scan_local_directories():
        ...
        # config.MODELS_ROOT already resolves the override, the WebUI's own path
        # config, and the extensions/<name>/../../models layout.
        if os.path.isdir(config.MODELS_ROOT):
            scanned = await asyncio.to_thread(
                scan_directories, config.MODELS_ROOT, TYPE_BY_DIR, RUST_AVAILABLE
            )
    ...
    async def refresh_local_database():
        ...
        try:
            scanned_files = await asyncio.to_thread(
                refresh_database, config.MODELS_ROOT, RUST_AVAILABLE
            )
            return {
                "status": "ok",
